// HashingAberto.cpp : Defines the entry point for the console application.
//
// Hashing Linear ABERTO
...
bool verificaVetorAtingiuMetadeTAM(TAluno **vetor){
	TAluno *aluno;
	int cont = 0;
	for(int i =0; i <TAMVETOR; i++ ){
		if(	((*vetor)[i].matricula != 0) &&	((*vetor)[i].matricula != -1)){
			cont++;
		}
	}
	if(cont >= (TAMVETOR/2) ){
...
void aumentaVetor(TAluno **vetor){
	int ind = 0, indVetorAUX = 0;
	TAluno *vetorAUX, *aluno;
	vetorAUX = (TAluno*) malloc(TAMVETOR *sizeof(TAluno));
	inicializaVetor(&vetorAUX);

	//Passa o conteúdo de vetor para vetorAUX;
	for(int i =0; i <TAMVETOR; i++ ){
		if(	((*vetor)[i].matricula != 0) &&	((*vetor)[i].matricula != -1)){
			vetorAUX[indVetorAUX] = (*vetor)[i];
			indVetorAUX++;
		}
	}

	//(*vetor) = (TAluno *) realloc( (*vetor), (TAMVETOR)*sizeof(TAluno) );
	TAMVETOR = TAMVETOR + TAMVETOR;
	(*vetor) = (TAluno*) malloc(TAMVETOR *sizeof(TAluno));
	inicializaVetor(&(*vetor));

	//Passa o conteúdo de vetorAUX para vetor;
	for(int i =0; i < (TAMVETOR/2); i++ ){
		if(	((vetorAUX)[i].matricula != 0) &&((vetorAUX)[i].matricula != -1)){
			aluno = &(vetorAUX)[i];
			ind = (funcaoHashing(*aluno, &(*vetor), NULL, NULL, false));
			(*vetor)[ind] = (vetorAUX)[i];
			indVetorAUX++;
		}
	}
	//printf("Vetor Depois: %d \n", (TAMVETOR *sizeof(TAluno)) );
}
	...
	while( (fgets(texto, sizeof(texto), arquivo))!=NULL ){
		...
		switch (cont%5){
			...
			case 0:
				...
				// Verifica se o Vetor Atingiu 50%
				if(verificaVetorAtingiuMetadeTAM(&(*vetor)) == true){
					printf("Atingiu 50%");
					aumentaVetor(&(*vetor));
				}
				break;
